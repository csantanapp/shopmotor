...
const plans = [
  { name: "Turbo", tagline: "Mais visibilidade no feed por 3 dias", price: "R$ 49", features: ["3x mais visualizações", "Selo Turbo no anúncio", "Aparece no topo da categoria", "Recomendado para estoque novo"], cta: "Ativar Turbo", highlight: false },
  { name: "Destaque", tagline: "Posição premium por 7 dias", price: "R$ 129", features: ["5x mais visualizações", "Posição fixa no topo", "Inclusão em e-mail marketing", "Recomendado para alta intenção"], cta: "Ativar Destaque", highlight: true },
  { name: "Super Destaque", tagline: "Máxima exposição por 15 dias", price: "R$ 289", features: ["10x mais visualizações", "Banner na home do ShopMotor", "Prioridade em buscas", "Recomendado para veículos premium"], cta: "Ativar Super", highlight: false },
];
    ...
    <ErpLayout title="Impulsionamento Inteligente" subtitle="Aumente o alcance dos veículos certos, no momento certo">
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-10">
        {plans.map((p) => (
          <div key={p.name} className={`rounded-xl border p-6 flex flex-col shadow-sm ${p.highlight ? "border-primary-container/50 bg-yellow-50" : "border-black/10 bg-white"}`}>
            ...
            <button onClick={() => fire(`Plano ${p.name} ativado`)} className={`mt-6 w-full rounded-xl py-2.5 text-sm font-black transition ${p.highlight ? "bg-primary-container text-black hover:opacity-90" : "border border-black/10 text-gray-700 hover:bg-gray-100"}`}>
              {p.cta}
            </button>
          </div>
        ))}
      ...
      <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-4 mb-6 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-yellow-800">Veículos com alto volume de visitas, favoritos e leads recentes — momento ideal para investir em alcance.</p>
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-3 mb-10">
        {recommended.map((r) => (
          <div key={r.car} className="rounded-xl border border-black/10 bg-white p-5 shadow-sm">
            <p className="text-xs uppercase tracking-wider text-yellow-700 font-black">Score {r.score}</p>
            <h4 className="mt-1 font-black text-gray-900">{r.car}</h4>
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              {[{ icon: "visibility", val: r.views.toLocaleString("pt-BR") }, { icon: "favorite", val: r.favs }, { icon: "phone", val: r.leads }].map((s, i) => (
                <div key={i} className="rounded-md bg-gray-50 border border-black/5 py-2">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {ads.map((a) => (
          <div key={a.car} className="rounded-xl border border-black/10 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div className="min-w-0 flex-1">
                <p className="text-xs text-gray-400 uppercase tracking-wider">Anúncio</p>
                <h4 className="mt-1 font-black text-gray-900 truncate">{a.car}</h4>
              </div>
