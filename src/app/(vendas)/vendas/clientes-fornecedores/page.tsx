...
const EMPTY = { tipo: "PF", nome: "", documento: "", telefone: "", email: "", endereco: "", cidade: "", estado: "", cep: "" };
...
export default function ClientesFornecedoresPage() {
  ...
  async function handleSave() {
    ...
    fire("Cliente/Fornecedor cadastrado!");
    setShowModal(false);
    setForm({ ...EMPTY });
    await load();
  }
  ...
  const pf = items.filter(i => i.tipo === "PF").length;
  const pj = items.filter(i => i.tipo === "PJ").length;
  ...
  return (
    <ErpLayout title="Clientes / Fornecedores" subtitle="Cadastro de clientes e fornecedores da sua loja">
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-black/10">
              <p className="font-black text-gray-900">Novo Cliente / Fornecedor</p>
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        ...
        <ErpKpiCard label="Pessoa Física" value={String(pf)} icon="person" />
        <ErpKpiCard label="Pessoa Jurídica" value={String(pj)} icon="business" />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <Icon name="people" className="text-5xl text-gray-200 mb-4" />
          <p className="text-lg font-black text-gray-400">{search ? "Nenhum resultado" : "Nenhum cadastro ainda"}</p>
          {!search && <p className="text-sm text-gray-400 mt-1">Clique em "Novo cadastro" para adicionar.</p>}
        </div>
      ) : (
...
/* shared form used in modal and detail page */
export function ModalForm({ form, setF }: { form: Record<string, string>; setF: (f: string, v: string) => void }) {
  return (
