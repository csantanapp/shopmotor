...
const scoreLabel = (s: number) =>
  s >= 80 ? { label: "Alta chance", cls: "bg-green-500/15 text-green-400 border-green-500/30" }
  : s >= 60 ? { label: "Média chance", cls: "bg-primary-container/20 text-primary-container border-primary-container/30" }
  : { label: "Baixa chance", cls: "bg-red-500/15 text-red-400 border-red-500/30" };
...
export default function VeiculosPage() {
  ...
  return (
    ...
    >
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 rounded-xl bg-[#1a1a1a] border border-white/10 px-4 py-3 text-sm text-white shadow-2xl">
          {toast}
        </div>
      )}

      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#1a1a1a] px-3 py-2 flex-1 min-w-64">
          <Icon name="search" className="text-sm text-white/40" />
          <input placeholder="Buscar por modelo, marca…" className="flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/30" />
        </div>
        {["Marca", "Status", "Score"].map((f) => (
          <button key={f} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-[#1a1a1a] px-3 py-2 text-sm text-white/70 hover:bg-white/5">
            <Icon name="filter_list" className="text-sm" /> {f}
          </button>
        ))}
      </div>

      <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4 mb-6 flex items-start gap-3">
        <Icon name="auto_awesome" className="text-blue-400 text-lg shrink-0 mt-0.5" />
        <div>
          <p className="text-sm font-bold text-white">Anúncios com mais fotos tendem a converter melhor</p>
          <p className="text-xs text-white/50">Veículos com 8+ fotos recebem em média 2,3x mais contatos.</p>
        </div>
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {vehicles.map((v) => {
          ...
          return (
            <div key={v.id} className="rounded-xl border border-white/10 bg-[#1a1a1a] p-5 flex flex-col">
              <div className="flex items-start gap-3">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-white/5 text-primary-container">
                  <Icon name="directions_car" className="text-2xl" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <ErpStatusBadge status={v.status} />
                    <span className="text-[11px] text-white/40">{v.daysInStock}d em estoque</span>
                  </div>
                  <h3 className="font-black text-white truncate">{v.name}</h3>
                  <p className="text-xs text-white/40">{v.brand} · {v.year}</p>
                </div>
              </div>

              <div className="mt-4 flex items-baseline justify-between">
                <p className="text-xl font-black text-white">R$ {v.price.toLocaleString("pt-BR")}</p>
                <p className="text-[11px] text-white/40">FIPE R$ {v.fipe.toLocaleString("pt-BR")}</p>
              </div>

              <div className="mt-4 rounded-lg border border-white/10 bg-white/5 p-3">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] font-black uppercase tracking-wider text-white/40">Score de venda</p>
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-2xl font-black text-white">{v.score}</span>
                ...
                <ul className="mt-3 space-y-1.5">
                  {v.scoreBreakdown.map((b) => (
                    <li key={b.label} className="flex items-center gap-2 text-[11px]">
                      <span className="w-24 text-white/40">{b.label}</span>
                      <div className="h-1 flex-1 rounded-full bg-white/10">
                        <div className="h-full rounded-full bg-primary-container/70" style={{ width: `${b.value}%` }} />
                      </div>
                      <span className="w-7 text-right font-bold text-white">{b.value}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              {v.suggestions.length > 0 && (
                <div className="mt-4">
                  <p className="text-[11px] font-black uppercase tracking-wider text-primary-container mb-2">Sugestões automáticas</p>
