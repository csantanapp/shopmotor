...
const STATIC_UPDATED = "21 de abril de 2026";

async function getLGPD() {
  try {
    const rows = await (prisma as any).siteConfig.findMany({
      where: { key: { in: ["lgpd_privacidade", "lgpd_privacidade_updated"] } },
    });
    const config = Object.fromEntries(rows.map((r: { key: string; value: string }) => [r.key, r.value]));
    return {
      content: config.lgpd_privacidade || null,
      updated: config.lgpd_privacidade_updated || null,
    };
  } catch { return { content: null, updated: null }; }
}

export default async function PrivacidadePage() {
  const { content, updated } = await getLGPD();
  const lastUpdate = updated || STATIC_UPDATED;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12 space-y-10">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-on-surface-variant">Última atualização: {lastUpdate}</p>
      </div>

      {content ? (
        /* ── Conteúdo gerenciado pelo CMS ── */
        <div
          className="space-y-8 text-on-surface-variant text-sm leading-relaxed
            [&_h2]:text-base [&_h2]:font-black [&_h2]:text-on-surface [&_h2]:mt-6 [&_h2]:mb-2
            [&_p]:leading-relaxed
            [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:space-y-1
            [&_strong]:text-on-surface [&_strong]:font-black
            [&_a]:text-primary [&_a]:underline [&_a]:underline-offset-2
            [&_table]:w-full [&_table]:text-sm
            [&_th]:text-left [&_th]:font-black [&_th]:text-on-surface [&_th]:py-2 [&_th]:border-b [&_th]:border-outline-variant
            [&_td]:py-2 [&_td]:border-b [&_td]:border-outline-variant/50"
      ...
      <div className="pt-6 border-t border-outline-variant flex flex-wrap gap-4 text-sm">
        <Link href="/termos" className="text-primary font-semibold hover:underline">Termos de Uso</Link>
        <Link href="/" className="text-on-surface-variant hover:underline">Voltar ao início</Link>
      </div>
