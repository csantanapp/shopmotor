...
export async function GET(req: NextRequest) {
  ...
  const [total, last30Views, deviceRows, sourceRows, pageRows, cityRows] = await Promise.all([
    ...
    db.pageView.groupBy({ by: ["country"], _count: { id: true }, where: { createdAt: { gte: d30 }, country: { not: null }, ...notAdmin }, orderBy: { _count: { id: "desc" } }, take: 15 }),
  ]);
  ...
  return NextResponse.json({
    ...
    cities: cityRows.map((r: any) => ({ country: r.country, region: null, city: null, count: r._count.id })),
  });
}
