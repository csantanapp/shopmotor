...
const faqs = [
  ...
  { categoria: "Anúncios", pergunta: "Por quanto tempo meu anúncio fica ativo?", resposta: "Anúncios ficam ativos indefinidamente enquanto você não os pausar, excluir ou marcar como vendido. Anúncios impulsionados têm prazo de destaque conforme o plano escolhido.", pagina: "faq", ordem: 2 },
  ...
  { categoria: "Conta e Acesso", pergunta: "Meus dados pessoais são seguros?", resposta: "Sim. Seguimos a LGPD e adotamos criptografia de dados, HTTPS e boas práticas de segurança. Consulte nossa Política de Privacidade para mais detalhes.", pagina: "faq", ordem: 2 },
  ...
  // FIPE e Valores
  { categoria: "FIPE e Valores", pergunta: "Como é calculado o valor FIPE exibido?", resposta: "Consultamos a tabela FIPE via API parceira (Parallelum) com base na marca, modelo e ano do veículo informados no anúncio.", pagina: "faq", ordem: 1 },
...
async function main() {
  ...
  if (existing > 0) {
    console.log(`FAQ já populado (${existing} itens). Nada feito.`);
    return;
  }
  await (prisma as any).faqItem.createMany({ data: faqs });
  console.log(`✅ Seed concluído: ${faqs.length} perguntas inseridas.`);
}
